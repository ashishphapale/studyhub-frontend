...
async function fetchNotes() {
  ...
  try {
    ...
    showToast("Error fetching notes. Check server.", "danger");
  } finally {
    hideSpinner();
  }
}
...
function renderNotes(notes = []) {
  ...
  if (!notes.length) {
    notesContainer.innerHTML = `<div class="col-12 text-center text-muted">No notes yet — upload your first note.</div>`;
    return;
  }
    ...
    col.innerHTML = `
      <div class="note-card card h-100">
        <div class="card-body d-flex flex-column">
          ...
          <p class="note-meta mb-2">Tags: ${Array.isArray(note.tags) ? note.tags.join(", ") : (note.tags || "—")}</p>
          <div class="mt-auto d-flex gap-2">
            <a class="btn btn-outline-primary btn-sm flex-fill" href="http://localhost:5000/api/notes/download/${note._id}" target="_blank">📥 Download</a>
...
// small helper: escape HTML to avoid injection in titles
